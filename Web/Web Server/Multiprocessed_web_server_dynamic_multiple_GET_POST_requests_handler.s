...
_start:
        ...
        CHILD_PROCESS:
            ...
            CLIENT_REQUSET_HANDLER:
                ...
                EXTRACT_PATH:
                    ...
                    xor r15, r15                        # Clear r15 (to store 'G' or 'P').
                    mov r15b, byte ptr [rdi]
                    call EXTRACT_FILE_PATH

                # Check the result.
                test eax, eax                           # Get the result of the EXTRACT_FILE_PATH call.
                jnz EXIT_CHILD_WITH_ERROR               # If eax == 1, handle the not found case.
            
                cmp r15b, 'G'                           # Check if the request starts with 'G' (GET).
                je GET_HANDLER                          # Jump to GET_HANDLER if it matches.

                cmp r15b, 'P'                           # Check if the request starts with 'P' (POST).
                je POST_HANDLER                         # Jump to POST_HANDLER if it matches.

                jmp EXIT_CHILD_WITH_ERROR               # If neither, handle invalid method.
    ...
    EXTRACT_FILE_PATH:
        cmp r15b, 'G'                                   # Check if the request starts with 'G' (GET).
        je FIND_GET_PREFIX                              # Jump to FIND_GET_PREFIX if it matches.

        cmp r15b, 'P'                                   # Check if the request starts with 'P' (POST).
        je FIND_POST_PREFIX                             # Jump to FIND_POST_PREFIX if it matches.

        jmp NOT_FOUND                                   # If neither, handle invalid method.
        ...
        SKIP_CHAR:
            inc rdi
            jmp EXTRACT_FILE_PATH

        NOT_FOUND:
            mov eax, 1                                 # Set return value to 1 (not found).
            ret                                        # Return if "GET " is not found.
            
        EXTRACT_FILE_PATH_LOOP:
            cmp byte ptr [rdi], ' '
            je EXTRACT_FILE_PATH_END                   # If space is found, end of path.
            mov al, [rdi]
            mov [rdx], al
            inc rdi
            inc rdx
